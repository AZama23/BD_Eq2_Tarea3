...
class FunctionalDependency(Dependency):
    ...
    _CAPTURE_EXPRESSION = r"\{[A-z]+(?:,[A-z]+)*\}->\{[A-z]+(?:,[A-z]+)*\}" #str
    _SEPARATOR = "->"


    def is_trivial(self) -> bool:
        for attribute in self.dependant:
            if attribute not in self.determinant:
                return False
        return True
...
class MultivaluedDependency(Dependency):
    ...
    def is_trivial(self, heading: set[Attribute]) -> bool:
        ...
        union = self.determinant | self.dependant
        return union == heading
